...
class EdifyGenerator(object):
  ...
  def VerifyPartition(self, filename):
    """Check that the given partition (EMMC or MTD reference) has one of the
    given sha1 hashes. filename is in TYPE:PARTITION:SIZE:HASH format"""
    self.script.append('assert(verify_partition("%s")' % (filename,) +');')
